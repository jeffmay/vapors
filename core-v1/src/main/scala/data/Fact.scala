...
final case class SourceFactOfType[A](
  ...
  override def toString: String = s"SourceFact(${typeInfo.name}: ${typeInfo.simpleTypeName} = $value)"
}
...
final case class DerivedFactOfType[A](
  ...
  override def toString: String =
    s"DerivedFact(${typeInfo.name}: ${typeInfo.simpleTypeName} = $value, evidence = $evidence)"
}
