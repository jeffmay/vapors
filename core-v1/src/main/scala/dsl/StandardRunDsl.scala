...
trait StandardRunDsl extends RunExprDsl {
  ...
  override implicit final def run[O](expr: Nothing ~> O): RunStandardExpr[O] = new RunStandardExpr(expr)
  ...
  final class RunStandardExpr[+O](expr: Nothing ~> O) extends RunExpr(expr) {
