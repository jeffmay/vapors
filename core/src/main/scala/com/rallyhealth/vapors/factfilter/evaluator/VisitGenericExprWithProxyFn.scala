...
import com.rallyhealth.vapors.factfilter.evaluator.InterpretExprAsResultFn.Input
...
abstract class VisitGenericExprWithProxyFn[F[_] : Foldable, V, P, G[_]]
  ...
  override def visitDefine[M[_] : Foldable, T](expr: Expr.Define[M, T, P]): Input[F, V] => G[FactSet] = { input =>
  ...
  override def visitOutputWithinSet[R](expr: Expr.OutputWithinSet[F, V, R, P]): Input[F, V] => G[Boolean] =
    visitGeneric(expr, _)

  override def visitOutputWithinWindow[R](expr: Expr.OutputWithinWindow[F, V, R, P]): Input[F, V] => G[Boolean] =
    visitGeneric(expr, _)

  override def visitReturnInput(expr: Expr.ReturnInput[F, V, P]): Input[F, V] => G[F[V]] =
    visitGeneric(expr, _)
